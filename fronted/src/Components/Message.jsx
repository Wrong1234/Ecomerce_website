...
const ChatInterface = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const chats = usersData.users.map((user, index) => ({
        ...
        audio: user.audio ? `http://127.0.0.1:8000/storage/${user.audio}`: " ",
        lastMessage: user.lastMessage ? user.lastMessage : "No message yet",
  ...
  if (
      ...
      const newMessage = {
        ...
        audio: msg.audio ? `http://127.0.0.1:8000/storage/${msg.audio}` : " ",
      ...
      <div className="flex-1 flex flex-col">
       ...
       <div className="bg-white px-4 py-3">
            {!showRecorder ? (
              ...
              <VoiceMessage receiverId={receiver_id} onClose={() => setShowRecorder(false)} />
            )}
          </div>
